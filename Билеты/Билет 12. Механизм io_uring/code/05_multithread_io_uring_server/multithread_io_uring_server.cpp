...
void handle_accept(io_uring& ring, int server_fd, int worker_id, int res) {
  submit_accept(ring, server_fd);

  if (res < 0) {
    print_line("worker " + std::to_string(worker_id) +
               " accept failed: " + std::strerror(-res));
    return;
  }
  ...
  print_line("worker " + std::to_string(worker_id) +
             " accepted fd=" + std::to_string(conn->fd));
...
void worker_loop(int worker_id) {
  try {
    ...
    submit_accept(ring, server_fd);
    print_line("worker " + std::to_string(worker_id) + " listens on port " +
               std::to_string(kPort));
    ...
    while (true) {
      ...
      if (ret < 0) {
        print_line("worker " + std::to_string(worker_id) +
                   " wait_cqe failed: " + std::strerror(-ret));
        continue;
      }
      ...
      if (req->type == OpType::Accept) {
        handle_accept(ring, server_fd, worker_id, res);
    ...
    print_line("worker " + std::to_string(worker_id) + ": " + e.what());
  }
}
